  ...
  const handlePrimaryAction = () => {
    if (onPrimaryAction) {
      ...
      toast({
        ...
        description: `Action for "${title}" will be available in the next release.`,
      });
    }
  };
  ...
  const handleSecondaryAction = () => {
    if (onSecondaryAction) {
      ...
      toast({
        ...
        description: `Secondary action for "${title}" triggered.`,
      });
    }
  };
  
  return (
    <div className={`${bgColor} p-4 border rounded-lg flex items-start gap-3`}>
      ...
      <div className="flex-1">
        ...
        <div className="mt-2 flex gap-2">
          <button 
            className={`text-xs ${buttonBgColor} text-white px-3 py-1 rounded-full hover:opacity-90 transition-opacity`}
